...
void Contact::applyPositionChange(Vector3 linearChange[2], Vector3 angularChange[2], float penetration)
{
	...
	for(unsigned i = 0; i < 2; i++)
	{
		if(m_body[i])
		{
			...
			projection.addScaledVector3(m_contactNormal, -m_relativeContactPosition[i].scalarProduct(m_contactNormal));
			...
			pos.addScaledVector3(m_contactNormal, linearMove[i]);
