...
void RigidBodyComponent::input(const Input& input, float delta)
{
	...
	{
		m_obj->getCollider()->getBody()->setAwake();
		//m_obj->getCollider()->m_body->setAcceleration(Vector3(0, 1.0f, 0));
		m_obj->getCollider()->getBody()->addForce(Vector3(0, -9.8f * m_obj->getCollider()->getBody()->getMass(), 0));
	}
	...
	{
		m_obj->getCollider()->getBody()->setAwake();
		m_obj->getCollider()->getBody()->addTorque(AXIS_X * toRadians(100));
	}
}
